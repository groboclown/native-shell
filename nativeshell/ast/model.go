...
//
// The action doesn't need to be named explicitly by the end user, but one must be assigned by
// the parser.  It's how the global and local variable names are generated.
...
// Ref the name of the thing itself, or a reference to the thing.
type Ref string

// Parameters maintain a relationship between a distinct name and a possible value type.
type Parameters map[string]LinkedParameter

// LinkedParameter is the input to a named parameter, which may be a user value or a field from another template.
type LinkedParameter struct {
	Field *FieldValue
	Value *ParameterValue
}

// FieldValue references a field generated by an action's template.
